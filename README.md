...
### 1. アプリ埋め込み秘密鍵の抽出リスク（リバースエンジニアリング）
現在の実装では、ビルド時に環境変数（`local.properties` または GitHub Secrets）からアプリの `BuildConfig.SPOTLOCK_PRIVATE_KEY` に秘密鍵を直接埋め込んでいます。
* **リスク**: アプリのAPKファイルを入手した悪意ある第三者が、デコンパイルツール（`JADX` や `apktool` など）を使用してバイナリを解析した場合、**埋め込まれている署名用の秘密鍵（Base64文字列）を簡単に抽出されてしまうリスク**があります。秘密鍵が漏洩すると、ダッシュボードでの検証を通過する不正な偽装画像を誰でも生成可能になります。
* **推奨される本番対策（Android Keystoreの採用）**:
  本番環境では、アプリ内に共通の秘密鍵を埋め込むのではなく、スマートフォン内のセキュリティチップ（TEEやStrongBoxなど）を利用する **Android Keystore System** を使用します。
  1. アプリ起動時に、デバイス固有のECDSA鍵ペアを端末内部で動的に生成させます（秘密鍵は端末外に取り出せなくなります）。
  2. 生成された端末固有の「公開鍵」を、事前に信頼できる管理サーバーに登録します。
  3. Webダッシュボード側は、サーバーに登録されたその端末の公開鍵を参照して写真の検証を行います。
...
ローカル開発環境とGitHub Actions（本番・デプロイ環境）の双方で、以下の通り鍵の更新作業を行います。

#### 1. 鍵ペアの生成
`python generate_keys.py` で新しいペア（秘密鍵Base64、公開鍵Hex）を作成します。

#### 2. GitHubの変数・シークレットの更新
GitHubリポジトリの設定（Settings ＞ Secrets and variables ＞ Actions）から以下の項目を更新します。これらを設定することで、ソースコードを変更することなく自動で新しい鍵によるアプリビルドとWebデプロイが完了します。
* **Secrets (暗号化シークレット)**:
  * `SPOTLOCK_PRIVATE_KEY`: 新しい秘密鍵（Base64文字列）に更新。
* **Variables (変数)**:
  * `SPOTLOCK_KEY_VERSION`: 新しいバージョン番号（例: `2`）に更新。
  * `SPOTLOCK_PUBLIC_KEYS`: 歴代の公開鍵のJSONリストに新しいバージョンと公開鍵（Hex）を追記（例: `{"1": "初代公開鍵", "2": "二代目公開鍵"}`）。

#### 3. ローカル開発環境の更新 (手動)
ローカルでテストを行う場合は、以下のファイルを書き換えます。
* **`local.properties`**: `spotlock.privateKey` を新しい秘密鍵に、`spotlock.keyVersion` を新しいバージョン（例: `2`）に更新します。
* **`web/src/utils/crypto.js`**: `PUBLIC_KEYS` 内のフォールバック値（ローカル検証用）に、新しいバージョンと公開鍵（Hex）を追記します。
